    ...
    for szl_id in szl_dict.keys():
        ...
        for item in entities:
            if len(item) > 2:
                ...
                """TRAZAK - STORE DEVICE'S INFORMATION"""
                valuesToStore[index] = ("%s" % formated_item)
                size = size+1
                print ("valuesToStore[index] %s" %valuesToStore[index])
                index = index + 1
        if size > 2:
            StoreDevicesData(valuesToStore,ip,port)
            size = 0
...
def StoreDevicesData (res,ip,port):
    ...
    for i in range(len(res)):
        value = '"'+str(res[i])+'"'
        if i == module_name:
            if res[i] != 0 and res[i] != "":
                sentence = string.replace(sentence, "&module_name&", value)
        if i == plant_id:
            if res[i] != 0 and res[i] != "":
                sentence = string.replace(sentence, "&plant_id&", value)
        if i == copyright:
            if value != 0 and value != "":
                sentence = string.replace(sentence, "&copyright&", value)
        if i == module_serial:
            if value != 0 and value != "":
                sentence = string.replace(sentence, "&module_serial&", value)
        if i == module_type:
            if value != 0 and value != "":
                sentence = string.replace(sentence, "&module_type&", value)
        if i == module_oem_id:
            if value != 0 and value != "":
                sentence = string.replace(sentence, "&module_oem_id&", value)
        sentence = string.replace(sentence, "&ip&", ip_str)
        sentence = string.replace(sentence, "&scanning_port&", port_str)
    print(sentence)
